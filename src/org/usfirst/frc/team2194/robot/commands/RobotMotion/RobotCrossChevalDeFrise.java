 ...
 *
 */
public class RobotCrossChevalDeFrise extends Command {
	...
	public RobotCrossChevalDeFrise(double distance, double speed,
	...
	protected void execute() {
		...
		if (!on2Dn
				&& (Robot.gyroRotate.getPitch() > Robot.prefs.getDouble(
						"Chdf ManUp", 3))) {
			Robot.manipulators.portcullisChevalManipulatorRaise();
			on2Dn = true;
		}

		if (Robot.gyroRotate.getPitch() < -3) {
			Robot.rightSideDrive.setMaxOut(Robot.currentMaxSpeed / 2);
			Robot.leftSideDrive.setMaxOut(Robot.currentMaxSpeed / 2);
